...
function sel_input(event, selnum)
...
function handle_lightshow_input(event)
...
function handle_kong_input(event, button)
...
function handle_keyboard_input(event, button)
	if(get_current_page() == "Keyboard") then
		buttonname = get_button_name(button)
		keynote = button_to_keynote[buttonname]
		if(keynote ~= nil) then
			remote.handle_input({ time_stamp = event.time_stamp, item = 1, value = 1, note = (keynote+12*g_basekey), velocity = button.z })
			return true
		end
	end
end

function handle_item_input(event, button)
...
function handle_helpmode_input(event, button)
...
function handle_valuemode_input(event, button)
	for buttonname,buttonmidi in pairs(buttons) do
		local color
		local button = remote.match_midi(buttonmidi.." 00", event)
...
function handle_internalpage_input(event, button)
